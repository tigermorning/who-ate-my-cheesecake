...
export function blit(sf, sheet, sx, sy, sw, sh, dx, dy, dw, dh, tint) {
...
export function drawObj(sf, key, tx, ty, cw, ch, TS) {
  ...
  let cursor = 0;
  for (const p of spec) {
    const w = p.rep === 'x' ? Math.max(1, cw - fixed) : (p.w ?? cw);
    const h = p.h ?? ch;
    ...
    if (p.rep === 'x') {
      ...
      for (let k = 0; k < w; k += unit)
        blit(sf, p.s, p.c * CELL, p.r * CELL, p.cw * CELL, p.ch * CELL,
             (tx + x + k) * TS, (ty + y) * TS, Math.min(unit, w - k) * TS, h * TS, p.tint);
    } else if (p.rep === 'y') {
      const unit = Math.max(1, p.ch);
      for (let k = 0; k < h; k += unit)
        blit(sf, p.s, p.c * CELL, p.r * CELL, p.cw * CELL, p.ch * CELL,
             (tx + x) * TS, (ty + y + k) * TS, w * TS, Math.min(unit, h - k) * TS, p.tint);
    } else {
      blit(sf, p.s, p.c * CELL, p.r * CELL, p.cw * CELL, p.ch * CELL,
           (tx + x) * TS, (ty + y) * TS, w * TS, h * TS, p.tint);
    }
