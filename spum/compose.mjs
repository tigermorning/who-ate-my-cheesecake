...
import { W, H, PROPS, ONWALL, PATCH, RUGS,
         floorOf, isWall, isFence, isDoor, isRoad, sizeOf, PASSABLE, at } from './house.mjs';
...
export function composeHouse(Z = 2, { shadows = true } = {}) {
  const TS = 16 * Z;
  const sf = surface(W * TS, H * TS, '#1E2A16');
  ...
  for (const p of items) {
    ...
    if (!drawObj(sf, p.key, p.x, p.y, p.cw, p.ch, TS))
