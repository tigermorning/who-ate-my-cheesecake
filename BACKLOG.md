...
- [x] **인격을 SPUM Cast 정본으로** (08-19) — `cast.json` persona 에 `traits`/`occupation`/`profile` 채움.
      NPC 프롬프트는 MBTI·경향·버릇·말투·내력·관계를 Cast 에서 읽는다. (08-20 CHARACTER_SYSTEM.md 판으로 교체)
...
- [x] **Step 5: NPC 기억 + 소통** (08-20 실측 검증 완료) — `spum/memory.mjs` + `dialogue.mjs`
      · 기억: 씨앗(자기 동선·목격·남의 사정) → 마주치면 한 조각씩 전달, 받은 쪽은 **출처**를 단다
      · 범인은 범행 시각 자기 자리를 안 흘린다. 대신 **알리바이 거짓말**(`round.alibiLie`)을 민다
      · 플레이어의 질문도 `asked` 로 기억돼 퍼진다
      · **엿듣기** — 플레이어가 5칸 안이면 소문이 대화창에 오른다 (대화 예산 안 씀)
      · 소문 문장은 **SAM 이 짓는다** (`buildGossipMessages`). 사실은 memory 가 고른 한 조각으로 고정
      · 증언판(`harvest`) 유연한 시각 파싱(정규화 및 방 행적 매칭) 및 다이렉트 대사 프롬프트 개선
      실측: `node spum/check-step5.mjs --close` 성공, 소문 전달, 질문/답변, 증언판 기록 2/36, 콘솔 오류 0

- [x] **Step 6: 맵 통합 (LANDMARKS 연결)** (08-20)
      · 12개 랜드마크(냉장고, 식탁, 난로, 서재 책상, 책장, 현관, 데크, 운동장, 텃밭, 헛간, 아일랜드, 소파)에 조사 설명(`desc`) 추가
      · 맵 spawnPoints 및 SPUM 맵 로더(`spummap.mjs`)에 랜드마크 메타데이터 전달
      · 플레이어가 랜드마크 근처에서 E키 또는 화면 클릭 시 `[조사]` 단서 확인 가능
      · 랜드마크 근접 시 오버레이 프롬프트 표시

- [x] **Step 7: 전체 데모 검증** (08-20)
      · 맵 및 이동: `node spum/verify-map.mjs` (길찾기 15/15 성공, 막힌 칸 진입 0, 플레이어 252칸 이동 오류 0)
      · 조작 및 자율배회: `node spum/verify-controls.mjs` (WASD/화살표, E 조사/대화, Space, Esc 이동복귀, Tab 증언판 여닫기, 12초 5/5 NPC 자율 이동)
      · 라운드 로직: `node spum/roundtest.mjs` (4000판 검증 실패 0)
      · 전체 콘솔 오류 0, 네트워크 실패 0

## 지금 하는 것
- 전체 기능 통합 및 검증 완료, 마케팅 데모 플레이 준비 완료

## 다음 (순서대로)
- [ ] 추가 마케팅용 시나리오/연출 다듬기 또는 브라우저 플레이 테스트
